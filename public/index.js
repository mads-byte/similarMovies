...
async function populateResults() {
    ...
    searchResults.forEach(movie => {
        ...
        movieDiv.innerHTML = `<div style="height: 231px; width: 154px; background-color: green"><img src="https://image.tmdb.org/t/p/w154/${movie.poster_path}"></div><div>${movie.title}<div><div>${movie.release_date}<div><div>${overview}</div><h2 style="display: block; font-size: 30px">Show Similar Movies</h2>`
        ...
        movieDiv.addEventListener('click', async () => {
            ...
            similarMovieResults.forEach(simMovie => {
                ...
                simMovieCard.innerHTML = `<div> style="height: 231px; width: 154px; background-color: green"<img src="https://image.tmdb.org/t/p/w154/${simMovie.poster_path}"></div><div>${simMovie.title}</div><div>${simMovie.release_date}</div><div>${overview}</div>`
